...
function renderTierlist() {
    ...
    // Only build role header if the feature is enabled
    ...
    tiers.forEach((tierName, i) => {
        ...
        if (currentGame.features.role) {
            ...
            currentGame.roles.forEach(role => {
                ...
                if (entries[i]) {
                    entries[i]
                        ...
                        .filter(Boolean)
                        .filter(c => c.role === role.name) // only characters for this role
                        .filter(applyFilters)
            ...
            // Role columns disabled → single column for all characters
            const column = document.createElement("div")
