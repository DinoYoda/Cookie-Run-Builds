...
function renderTierlist() {
    ...
    buildRoleHeader(tierlistContainer)  // can also check feature flag inside this
    ...
    tiers.forEach((tierName, i) => {
        ...
        if(currentGame.features.role) {
            ...
            currentGame.roles.forEach(role => {
                ...
                if(entries[i]) {
                    entries[i]
                      ...
                      .filter(Boolean)
                      .filter(c => c.role === role.name) // only if role is defined
                      .filter(applyFilters)
            ...
            // role columns disabled → single column with all characters
            const column = document.createElement("div")
