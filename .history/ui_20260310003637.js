...
function buildFilters(){
    ...
    Object.entries(filters).forEach(([category, values])=>{
        ...
        values.forEach(value=>{
            ...
            btn.title = value
            btn.innerHTML = `<img src="${iconPath}" alt="${value}">`
...
resetBtn.onclick = ()=>{
    ...
    searchText = ""
    searchInput.value = ""
    document.querySelectorAll(".filter-btn, .filter-icon-btn").forEach(btn=>btn.classList.remove("active"))
    renderTierlist()
}
...
function applyFilters(character){
    ...
    for(const [category,value] of Object.entries(activeFilters)){
        if(character[category] != value) return false
    }
    return true
}
...
function createCard(char){
    ...
    card.innerHTML = `
        <div class="portrait">
            <img src="${char.image || 'icons/default.png'}" class="character-img">
            ${char.elementIcon ? `<img class="element-icon" src="${char.icon}">` : ''}
            ${char.eidolon ? `<div class="eidolon">E${char.eidolon}</div>` : ''}
        </div>
        <div class="name">${char.name}</div>
        ${char.tags ? `<div class="tags">${char.tags.join(", ")}</div>` : ''}
    `
    return card
}