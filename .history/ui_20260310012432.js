...
function renderTierlist() {
    ...
    // Only build role header if roles enabled
    if (currentGame.features.role) buildRoleHeader(tierlistContainer)

    const tiers = currentTierlist.tiers
    const entries = currentTierlist.entries

    let totalCards = 0 // counter
    ...
    // Update counter
    document.getElementById("cardCounter").textContent = `${totalCards} card${totalCards === 1 ? '' : 's'}`
}
/* -----------------------------
CHARACTER CARD
----------------------------- */
