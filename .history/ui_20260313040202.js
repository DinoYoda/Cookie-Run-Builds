...
function createCard(char) {
    ...
    if (currentGame.features && currentGame.features.builds) {

        const slug = char.name.toLowerCase().replace(/\s+/g, "-")
        link = `builds/${currentGame.id}/${slug}.html`
    ...
    let html = `<a class="portrait" href="${link}" ${newTab}><img src="${imgSrc}" class="character-img">`

    if (currentGame.features && currentGame.features.elementIcon && char.icon) {
        html += `<img class="element-icon" src="${char.icon}">`
    }

    if (currentGame.features && currentGame.features.eidolon) {
    ...
    if (currentGame.features && currentGame.features.tags && char.tags) {
        html += `<div class="tags">${char.tags.join(", ")}</div>`
    }

    if (currentGame.features && currentGame.features.badges && char.badges) {
...
function renderCharacterPage(gameId, charSlug) {

    const game = DATA.games.find(g => g.id === gameId)

    const char = game.characters.find(c =>
        c.name.toLowerCase().replace(/\s+/g,"_") === charSlug
    )
    ...
    <div class="char-top">
        ...
        <img class="char-image" src="${getCardImagePath(char.name)}">
