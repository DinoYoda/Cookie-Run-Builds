...
function saveUIState() {
    localStorage.setItem("tierlistGame", currentGame?.id || "")
    localStorage.setItem("tierlistGroup", currentGroup?.name || "")
    localStorage.setItem("tierlistSubTab", currentSubTab?.name || "")
}
...
function buildGameSelector() {
    ...
    const savedGame = localStorage.getItem("tierlistGame")

    if (savedGame && DATA.games.find(g => g.id === savedGame)) {
        loadGame(savedGame)
    } else {
...
function buildTabs() {
    ...
    tierTabs.innerHTML = ""
    const savedGroup = localStorage.getItem("tierlistGroup")
    const savedSub = localStorage.getItem("tierlistSubTab")
...
if (!currentGroup) {
    ...
    const foundGroup = currentGame.tierlists.find(t => t.name === savedGroup)

    if (foundGroup) {
        currentGroup = foundGroup

        if (foundGroup.tierlists) {

            const foundSub = foundGroup.tierlists.find(s => s.name === savedSub)

            currentSubTab = foundSub || foundGroup.tierlists[0]
            currentTierlist = currentSubTab

        } else {
            currentTierlist = foundGroup
        }

    } else {

        currentGroup = currentGame.tierlists[0]

        if (currentGroup.tierlists) {
            currentSubTab = currentGroup.tierlists[0]
            currentTierlist = currentSubTab
        } else {
...
function getCardImagePath(name, id) {
    if (id === 'crk') {
        ...
        newName = newName.replace(/\s+/g, "_")
        return `icons/${newName}_mc_lv3.png`
    }
}
...
function createCard(char) {
    ...
    const wikiLink = getWikiLink(char.name, currentGame.id)

    let html = `<a class="portrait" href="${wikiLink}" target="_blank"><img src="${imgSrc}" class="character-img">`
    ...
    html += `</a><div class="name">${char.name}</div>`
    ...
    // Create the card element before using it
