...
const _TAG_RE = /(ice|fire|status|light|dark|color|steel|darkness|poison|water|wind|grass|electric|chaos|earth|header|cookie|treasure|skill|type)(-header)?\{([^}]*)\}/g
const _EL_ICONS = { ice: "Ice", fire: "Fire", light: "Light", dark: "Darkness", steel: "Steel", poison: "Poison", water: "Water", wind: "Wind", grass: "Grass", electric: "Electric", chaos: "Chaos", earth: "Earth", darkness: "Darkness" }
...
function tagParser(text) {
  ...
  return text.replace(_TAG_RE, (_, tag, noIcon, content) => {
    ...
    if (tag === "cookie") return `<img src="pictures/icons/cookie/${_esc(content.trim())}_head.png" alt="${_esc(content.trim())}" class="skill-status-icon" onerror="this.style.display='none'">`
    if (tag === "treasure") return `<img src="pictures/treasures/Treasure_${_esc(content.trim())}.png" alt="${_esc(content.trim())}" class="skill-status-icon" onerror="this.style.display='none'">`
    if (tag === "skill") { const s = content.trim(); const fn = s + "_skill"; return `<img src="pictures/skills/${_esc(fn)}.png" alt="${_esc(s)}" class="skill-status-icon" onerror="this.style.display='none'">` }
    if (tag === "status") {
      ...
      let html = `<img src="pictures/icons/status/status_${_esc(mainIconName)}.png" alt="${_esc(mainId)}" class="skill-status-icon" onerror="this.style.display='none'">`
      if (overlay === "und_debuff" || overlay === "und_buff") {
        const ovName = overlay === "und_debuff" ? "Undispellable_Debuff" : "Undispellable_Buff"
        html = `<span class="skill-status-icon-wrap"><img src="pictures/icons/status/status_${ovName}.png" alt="${_esc(overlay)}" class="skill-status-icon skill-status-icon-overlay" onerror="this.style.display='none'">${html}</span>`
      }
      if (element) {
        const elIconName = _EL_ICONS[element.toLowerCase()] || (element.charAt(0).toUpperCase() + element.slice(1))
        const elImg = `<img src="pictures/icons/${_esc(elIconName)}.png" alt="${_esc(element)}" class="skill-status-icon skill-status-icon-element" onerror="this.style.display='none'">`
        html = `<span class="skill-status-icon-wrap">${html}${elImg}</span>`
      }
      return html
    }
    if (tag === "type") { const t = content.trim(); return `<img src="pictures/icons/${_esc(t)}.png" alt="${_esc(t)}" class="skill-status-icon" onerror="this.style.display='none'">` }
    if (tag === "color") {
    ...
    return `<img src="pictures/icons/${_esc(iconName)}.png" alt="${_esc(tag)}" class="skill-status-icon text-tag" onerror="this.style.display='none'">${span}`
  })
}
...
function renderSkillTaggedText(tagged, skillAttr, levelIndex) {
  ...
  text.split(/<br>/).forEach((line) => {
    ...
    if (isHeader) {
      flushUl()
      parts.push(`<div class="char-skill-details-line">${tagParser(line)}</div>`)
    } else {
      let stripped = line
      let indentLevel = 0
...
function getPageImagePath(name) {
    return `pictures/chars/${name}_illustration.png`
}

function getToppingImagePath(type, resonance, isTart) {
  if (isTart) return `pictures/toppings/tart/Topping_tart_${type}_3.png`
  if (resonance) return `pictures/toppings/${type}/Topping_${type}_${resonance.toLowerCase()}.png`
  return `pictures/toppings/${type}/Topping_${type}_3.png`
}
...
function buildToppingsSetBlockHtml(topSet) {
  ...
  if (tart) {
    starHtml += `<img src="${tart.src}" data-fallback-src="${tart.fallbackSrc || ""}" alt="${tart.type}" class="char-topping-tart-base" onerror="if(!this.dataset.fallbackDone && this.dataset.fallbackSrc){this.dataset.fallbackDone='1';this.src=this.dataset.fallbackSrc;}else{this.src='pictures/toppings/unknown.png';}">`
  }
  regularToppings.forEach((t, i) => {
    starHtml += `<div class="char-topping-star-slot char-topping-pos-${i + 1}"><img src="${t.src}" data-fallback-src="${t.fallbackSrc || ""}" alt="${t.type}" class="char-topping-slot" onerror="if(!this.dataset.fallbackDone && this.dataset.fallbackSrc){this.dataset.fallbackDone='1';this.src=this.dataset.fallbackSrc;}else{this.src='pictures/toppings/unknown.png';}"></div>`
  })
  starHtml += `</div>`
...
function buildBeascuitSetBlockHtml(biscuitSet, charData) {
  ...
  const baseOverlay = el
    ? `<img src="pictures/beascuit/Beascuit_base_${cookieType}_${baseNumber}.png" alt="${el} base" class="char-beascuit-base-overlay" onerror="this.style.display='none'">`
    : ""
  const beascuitHtml = `<div class="char-beascuit-image-wrapper">
    <img src="pictures/beascuit/Beascuit_${cookieType}_legendary.png" alt="${el}" class="char-beascuit-icon" onerror="this.style.display='none'">
    ${baseOverlay}
  </div>`
...
async function renderCharacterPage(){
    ...
    if (data && data.games) {
        const game = data.games.find(g => g.id === "crk")
    ...
    const game = window.CRK_DATA?.games?.find(g => g.id === "crk")
    ...
    if(charData && infoBox) {
        ...
        const rarityIconPath = rarity ? `pictures/icons/${rarity}.png` : ""
        const typeRow = type ? `<div class="char-stat-pill"><img src="pictures/icons/${type}.png" alt=""><span>${type}</span></div>` : ""
        const posRow = position ? `<div class="char-stat-pill"><img src="pictures/icons/${position}.png" alt=""><span>${position}</span></div>` : ""
        const elemRow = elements.length ? `<div class="char-stat-pill"><span>Element</span>${elements.map(e => `<img src="pictures/icons/${e}.png" alt="${e}" title="${e}">`).join("")}</div>` : ""
        infoBox.innerHTML = `
            ${rarity ? `<img class="char-rarity-icon" src="${rarityIconPath}" alt="${rarity}" title="${rarity}">` : ""}
    ...
    function renderSkillSectionContent() {
        ...
        function skillBox(name, cooldown, initialCd, desc, iconPath, hasData, skillDetails, skillAttrData, lidx, detailsKey, middleContent) {
            ...
            const src = hasData ? iconPath : "pictures/skills/unknown.png"
            ...
            const cdPills = cooldown != null
                ? `<span class="char-skill-cd-pills">
                    <span class="char-skill-cd-pill" data-tooltip="Base CD"><img src="pictures/icons/clock.png" alt="" class="char-skill-clock">${cooldown} sec</span>
                    ${icd ? `<span class="char-skill-cd-pill char-skill-icd-pill" data-tooltip="Initial CD"><img src="pictures/icons/clock.png" alt="" class="char-skill-clock char-skill-clock-muted">${icd} sec</span>` : ""}
                   </span>`
                : ""
            ...
            return `
                <div class="char-skill-box">
                    <div class="char-skill-header">
                        <img class="char-skill-icon" src="${src}" alt="" onerror="this.src='pictures/skills/unknown.png'">
        ...
        const normalBox = skillBox(
            ...
            `pictures/skills/${slug}_skill.png`,
            true,
        ...
        if (hasNormalRally && normalSkillDetailsRaw) {
            const base = renderSkillTaggedText(normalSkillDetailsRaw, charData?.skillAttr, 0)
            const max = renderSkillTaggedText(normalSkillDetailsRaw, charData?.skillAttr, 1)
        ...
        if (isCJ && hasCJ) {
            ...
            mcCjBox = skillBox(
                ...
                `pictures/skills/${slug}_cj_skill.png`,
                true,
                null,
                null,
                null,
            ...
            mcCjBox = skillBox(
                ...
                `pictures/skills/${slug}_mc_skill.png`,
                true,
        ...
        const cjBubble = hasMcCj ? `<div class="char-skill-mccj-wrap"><h4 class="char-skill-mccj-header">${mcCjLabel}</h4>${cjDisclaimer}<div class="char-skill-bubble"><div class="char-skill-content">${mcCjBox}${hasMcCj ? rallyHtml : ""}${isCJ && hasCJ ? cjSkillDetailsHtml : ""}${cjEnchantsHtml}${cjAscensionHtml}</div>${mcCjBar}</div>${gameplayNotesHtml || ""}</div>` : ""
